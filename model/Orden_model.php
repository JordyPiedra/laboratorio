...
public function select_all($EST_ORD="%",$COD_USUCRE='%')
      ...
      return $this->db->select("ID_ORD,COD_ORD,COD_CIR,COD_PER,EST_ORD
								  ...
								  (SELECT CED_PER PRIAPE_PER FROM PERSONA WHERE COD_PER = O.COD_PER) CEDULA",'ORDEN O', 
								  "EST_ORD LIKE '$EST_ORD' AND COD_USUCRE LIKE '$COD_USUCRE' ORDER BY FECCRE_ORD DESC", PDO::FETCH_NUM);

		}

public function select_allbYCED($EST_ORD="%",$COD_USUCRE='%',$CEDPER='%')
		{
      return $this->db->select("O.ID_ORD,O.COD_ORD,O.COD_CIR,O.COD_PER,O.EST_ORD
	  							,O.OBS_ORD,O.FECMUE_ORD,O.FECCRE_ORD,O.FECRES_ORD,O.COD_USUCRE,O.COD_USURES,
								  (SELECT CONCAT(PRINOM_PER, ' ', PRIAPE_PER) FROM PERSONA WHERE COD_PER = O.COD_PER) PACIENTE,
								  (SELECT CED_PER PRIAPE_PER FROM PERSONA WHERE COD_PER = O.COD_PER) CEDULA",'ORDEN O , PERSONA P', 
								  "P.COD_PER = O.COD_PER AND O.EST_ORD LIKE '$EST_ORD' AND O.COD_USUCRE LIKE '$COD_USUCRE' AND P.CED_PER LIKE '$CEDPER' ORDER BY FECCRE_ORD DESC", PDO::FETCH_NUM);
