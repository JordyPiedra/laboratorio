...
class PDOManager extends PDO{
        ...
        try {
                ...
                switch ($DB_TYPE) {
                    case 'mysql':
                        ...
                        break;
                    case 'sqlsrv':
                        parent::__construct($DB_TYPE.':server='.$DB_HOST.';Database='.$DB_NAME, $DB_USER, $DB_PASS);
                        break;
                    case 'oci':
                        parent::__construct($DB_TYPE.':dbname='.$DB_HOST.'/'.$DB_NAME.';charset=AL32UTF8', $DB_USER, $DB_PASS);
                        break;    
                    default:
                        printf("Connect failed: Base de datos no especificada ");
                        break;
                }
	...
	*/
	public function select($attr,$table,$where = '',$fetchMode = PDO::FETCH_ASSOC,$dot=true){
        $char= $dot ? ';' : '';  
		$where = ($where != '' ||  $where != null) ? "WHERE ".$where : '';
		$sql = "SELECT ".$attr." FROM ".$table." ".$where.$char;
		//echo $sql;
	...
	*/
	...
	function startTransaction ($SQLs, $flag=0){
		try {   
		    $this->beginTransaction();
                        
            
            if (!$flag) {
               //$result = $this->prepare($SQLs);
               //$result->execute();
               $this->exec($SQLs);
            }else{
               for ($i=0; $i < count($SQLs); $i++) { 
                    $result = $this->prepare($SQLs[$i]);
                    $result->execute();
                }
            }
